...
import com.mlex0.musicschoolandroidclient.Model.TeacherSchedule;
...
public class MainFrameActivity extends AppCompatActivity {
    ...
    private void ChangeByRole(){
        if(Constants.IDRole.equals("1")){
            BottomNav.getMenu().removeItem(R.id.nav_schedule);
            BottomNav.getMenu().removeItem(R.id.nav_marks);
            showChatsLayout();
        }
    ...
    private void updateSchedule(){
        ...
        if(Constants.IDRole.equals("2")){
            ...
            String url = Constants.ApiUrl + "schedule/teacheruser/" + Constants.UserId;
            ...
            StringRequest request = new StringRequest(Request.Method.GET, url, response -> {
                try {
                    ...
                    List<TeacherSchedule> scheduleItems = new ArrayList<>();
                    for(int i = 0; i < jsonArray.length(); i++){
                        JSONObject OneResponse = new JSONObject(jsonArray.get(i).toString());
                        TeacherSchedule scheduleItem = new TeacherSchedule();
                    ...
                    TeacherDrawSchedule(scheduleItems);
        ...
        if(Constants.IDRole.equals("3")){
            String url = Constants.ApiUrl + "schedule/groupuser/" + Constants.UserId;
    ...
    private void TeacherDrawSchedule(List<TeacherSchedule> scheduleItems) {

        for (TeacherSchedule scheduleItem : scheduleItems) {
            //Понедельник
            if (scheduleItem.getWeekDayNumber().equals("1") && scheduleItem.getDayOfWeek().equals("Понедельник")){
                ...
                tvTeacher.setText("Группа: " + scheduleItem.getGroupNumber());
    ...
    private void updateMarks(){
        ...
        String url = Constants.ApiUrl + "mark/studentall/" + Constants.UserStudent.getID();
    ...
    private void updateProfile() {
        ImageView imageProfile = findViewById(R.id.profile_image_main);
        TextView FIOProfile = findViewById(R.id.profile_username_main);
        TextView email = findViewById(R.id.email_main);

        String rightPath = Constants.UserImage.toString().replace("uploads\\", "");
        rightPath = rightPath.toString().replace("uploads/", "");
        ...
        Glide.with(this).load(glideUrl).into(imageProfile);

        email.setText(Constants.UserLogin);

        if(Constants.IDRole.equals("1")){
            FIOProfile.setText("Администратор");
        }
        if(Constants.IDRole.equals("2")){
            FIOProfile.setText(Constants.UserTeacher.getLastName() + " "
                    + Constants.UserTeacher.getFirstName() + " " + Constants.UserTeacher.getPatronymic());
        }
        if(Constants.IDRole.equals("3")){
            FIOProfile.setText(Constants.UserStudent.getLastName() + " "
                    + Constants.UserStudent.getFirstName() + " " + Constants.UserStudent.getPatronymic());
        }
